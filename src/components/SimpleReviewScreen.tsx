...
export const SimpleReviewScreen: React.FC<SimpleReviewScreenProps> = ({
  ...
  const [commentCopied, setCommentCopied] = useState<boolean>(false);
          ...
          {rating !== null && rating >= 4 && (
            <div className="space-y-4 text-center animate-fadeIn">
              <div className="p-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl text-left">
                ...
                <p className="text-[11px] text-emerald-800 leading-relaxed">Step 1: Choose or copy your preferred review comment below. Step 2: Click "Done" to open Google Review.</p>
              </div>

              {/* Active Selected Comment Box with 1-Click Copy */}
              <div className="text-left bg-white border border-slate-200 rounded-2xl p-3 shadow-sm">
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-[11px] font-bold text-slate-700 flex items-center gap-1.5">
                    ...
                    <span>Selected Comment (Tap to Copy):</span>
                  </label>
                  {commentCopied && <span className="text-[10px] font-bold text-emerald-600 flex items-center gap-1"><Check className="w-3 h-3" /> Copied</span>}
                </div>
                <div className="relative">
                  <textarea
                    ...
                    placeholder="Type or select your review comment..."
              ...
              {/* Pre-populated templates list for customer to choose & copy */}
              <div className="text-left pt-1">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-[11px] font-bold text-slate-700 block">Tap any template to copy:</span>
                  <span className="text-[10px] text-amber-600 font-bold">{reviewTemplates.length} Options</span>
                </div>
                <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                  {reviewTemplates.map((template, idx) => {
                    ...
                    return (
                      <div
                        key={idx}
                        onClick={() => handleCopyComment(template)}
                        className={`w-full p-3 rounded-2xl border text-left text-xs transition cursor-pointer flex items-center justify-between gap-2 ${isSelected ? 'bg-amber-50 border-amber-300 text-amber-950 shadow-2xs ring-1 ring-amber-400' : 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-700'}`}
                      >
                        <p className="text-xs italic leading-relaxed flex-1">"{template}"</p>
                        <button
                          type="button"
                          className={`px-2.5 py-1 rounded-lg text-[10px] font-bold shrink-0 flex items-center gap-1 transition ${isSelected ? 'bg-amber-500 text-slate-950' : 'bg-white border border-slate-200 text-slate-600'}`}
                        >
                          {isSelected ? <><Check className="w-3 h-3" /><span>Copied</span></> : <><Copy className="w-3 h-3" /><span>Copy</span></>}
                        </button>
                      </div>
                    );
                  })}
              ...
              {/* Done Button to Redirect to Google */}
              <button
                type="button"
                onClick={handleDoneAndRedirect}
                className="w-full py-4 px-5 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm rounded-2xl shadow-lg flex items-center justify-center gap-2.5 transition transform active:scale-95 cursor-pointer"
              >
                <CheckCircle2 className="w-5 h-5 text-white" />
                <span>Done - Go to Google Review</span>
